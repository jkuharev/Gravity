...
// 04 --
// 05 -- Bei m =  | x | ,n =  | y | ,n < m besitzt der Algorithmus eine Laufzeit von ��(nm)
// 06 -- und einen Speicherverbrauch von ��(min{n,m}).
// 07 --
...
// 15                        -- Konstruiere Matrix T f��r die Teil-Strings
...
// 26                -- mit dem Vorw��rtsalgorithmus:
// 27                T^\ell(j_1) := 0
...
// 72 begin
// 73        m :=  | x | ; n :=  | y | 
// 74        -- Sonderbehandlung: x ist der leere String und l��sst keine Zerteilung zu:
// 75        if m=0 then
 ...
 */
public class ParallelLinearRTW extends RTW implements ParallelAlignment<IMSPeak>
{
	private int initialRecursionDepth = 0;
	private TrivialRTW<IMSPeak> trivial = null;
	private LevenshteinDistanceCalculator<IMSPeak> levenshtein = null;
	private int maxRecursionDepthForThreadSplitting = 6;
	 ...
	 * @param topPeaks
	*/
	public ParallelLinearRTW(List<IMSPeak> leftPeaks, List<IMSPeak> topPeaks)
	{
	...
	{
		this.trivial = new TrivialRTW<IMSPeak>(matcher);
		this.levenshtein = new SimpleLevenshteinDistanceCalculator<IMSPeak>(matcher);
		aln = align(top, left, initialRecursionDepth);
	}
	
	@Override public void setMaxDepth(int maxDepth)
	{
		maxRecursionDepthForThreadSplitting = maxDepth;
	}
	
	/**
	 * ATTENTION: Hirschberg results in only one alignment path
	 * thus preferTopPath is ignored in this implementation
	 * @param preferTopPath IGNORED!!!
	 */
	@SuppressWarnings("unchecked") @Override public PairwiseAlignment getPeakAlignment(Object topRowID, Object leftRowID)
	{
	 ...
	 */
	@Override
	public LinkedAlignment<IMSPeak> align( XSubList<IMSPeak> topSeq, XSubList<IMSPeak> leftSeq, int recursionDepth )
    {
    	recursionDepth++;
        ...
        // on break condition find alignment using needleman wunsch (trivial case)
        if(n<=1 || m<=1) return trivial.getAlignment(topSeq, leftSeq, useLeftPath);
        ...
        // avoid unnecessary stack growing
        ...
        // dive into deeper recursion for solving subproblems
        LinkedAlignment<IMSPeak> head = null;
        LinkedAlignment<IMSPeak> tail = null;
        
		if (recursionDepth < maxRecursionDepthForThreadSplitting)
        {
			ParallelAlignmentThread<IMSPeak> headFork = new ParallelAlignmentThread<IMSPeak>( prefixY, prefixX, this, recursionDepth );
			ParallelAlignmentThread<IMSPeak> tailFork = new ParallelAlignmentThread<IMSPeak>( suffixY, suffixX, this, recursionDepth );
			head = headFork.getAlignment();
			tail = tailFork.getAlignment();
			headFork = null;
			tailFork = null;
        }
        else
        {
			head = align( prefixX, prefixY, recursionDepth );
			tail = align( suffixX, suffixY, recursionDepth );
        }
        
        // concatenate alignments and return
        return (head==null) ? tail : ( (tail==null) ? head : head.append(tail) );
    }
}

...
import de.mz.jk.ms.align.method.dtw.BandedRTW;
import de.mz.jk.ms.align.method.dtw.TrivialRTW;

/**
 * <h3>{@link ParallelFastLinearRTW}</h3>
 ...
 */
public class ParallelFastLinearRTW extends BandedRTW implements ParallelAlignment<IMSPeak>
{
	private int initialRecursionDepth = 0;
	private int maxRecursionDepth4ThreadSplitting = 6;
	 ...
	 * @param topPeaks
	*/
	public ParallelFastLinearRTW(List<IMSPeak> leftPeaks, List<IMSPeak> topPeaks)
	{
		super(leftPeaks, topPeaks);
		// ensure SCORE_EDGE is out of possible score value range 
		matcher.SCORE_EDGE = (left.size() + top.size()) * matcher.SCORE_GAP + 10 * matcher.SCORE_GAP;
	}

	@Override public void run()
	{
		this.trivialAligner = new TrivialRTW<IMSPeak>(matcher);
		this.levenshtein = new BandedLevenshteinDistanceCalculator<IMSPeak>(matcher, getColRanges());
		aln = align( top, left, initialRecursionDepth );
	}

	@Override public void setMaxDepth(int maxDepth)
	{
		maxRecursionDepth4ThreadSplitting = maxDepth;
	}
    ...
    public LinkedAlignment<IMSPeak> align( XSubList<IMSPeak> topSeq, XSubList<IMSPeak> leftSeq, int recursionDepth )
    {
        ...
        if(recursionDepth < maxRecursionDepth4ThreadSplitting)
        {
